Given(/^the user navigates to (.*) on browser$/) do |page|
  ...
  no_response_error = "This site can’t be reached"
  Env_Status = "Service Unavailable"

  if page_html.include? server_error
    fail("#{server_error}, is returned by the server, please check to make sure the server is up and running then try again")
  else
    if page_html.include? no_response_error
      fail("#{no_response_error}, is returned by the server, please check to make sure the server is up and running then try again")
    else
      if page_html.include? Env_Status
        fail("#{Env_Status}, is returned by the server, please check to make sure the server is up and running then try again")
      else
        puts "Sever is up and running"
      end
      clear_alert
    end
  end
...
Then(/^the switch to new tab page$/) do
  if @browser.windows.last.use
  puts ("The window last used ID is : #{@browser.windows.last.use}")
  else
    puts'No new window exist or open'
end
end
 ...
 Then(/^the return to main page$/) do
  ...
  # @browser.driver.switch_to.default_content
   if @browser.windows.first.use
     puts ("The window first used ID is : #{@browser.windows.first.use}")
   else
     puts'No new window exist or open'
   end
 end
