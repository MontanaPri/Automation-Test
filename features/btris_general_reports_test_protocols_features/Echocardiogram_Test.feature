...
Feature: BTRIS Echocardiogram Test Report
  ...
  #Login to the system.
    ...
    Examples:
      ...
      | 00-DK-0107 | btris_test2 | ARNETT           | Modified Bruce Protocol Exercise Tolerance Echocardiography, Procedure | Bruce Protocol Exercise Tolerance Echocardiography, Procedure | Exercise Tolerance Echocardiography Procedure by Standardized Protocol Used | Gardner Protocol Exercise Tolerance Echocardiography, Procedure | Manual Treadmill Protocol Exercise Tolerance Echocardiography, Procedure | Stress Echocardiography, Procedure |


  @Echocardiogram2

  Scenario Outline: 14.2 Echocardiogram Report Including echocardiogram Tests.
  #Login to the system.
    Given the user navigates to / on browser
    And the user must see "Log in to BTRIS" text in "btris home: landing page" field on "Btris/Portal" page
    #Login to the BTRIS system.
    ...
    #verify the select report page.
    And the user must see "Echocardiogram Criteria" text in "btris: echocardiogram report page" field on "Btris/Portal" page
    #Click on select Echocardiogram Test.Scenario:
    And the user enters "<Echocardiogram Test Components>" into "echocardiogram components search fields" on "Btris/Portal" page
    And the user clicks on "echocardiogram test components" element on "Btris/Portal" page
    #Clcik on the select report button.
    ...
    Examples:
      | Subject       | User_Name   | Filtered Subject | Echocardiogram Tests                                                   | Echocardiogram Test Components      |
      | BTRIS-TEST-04 | btris_test2 | NIHCCTEST        | Modified Bruce Protocol Exercise Tolerance Echocardiography, Procedure | Amended On, Echocardiography Report |


  @Echocardiogram_SC3

  Scenario Outline: 14.3 Echocardiogram Report Including All echocardiogram Tests, Components with Shopping cart.
  #Login to the system.
    Given the user navigates to / on browser
    And the user must see "Log in to BTRIS" text in "btris home: landing page" field on "Btris/Portal" page
    #Login to the BTRIS system.
    ...
    #verify the select report page.
    And the user must see "Echocardiogram Criteria" text in "btris: echocardiogram report page" field on "Btris/Portal" page
    #Click on select Echocardiogram Test Component.Scenario:
    And the user enters "<Echocardiogram Test Components>" into "echocardiogram components search fields" on "Btris/Portal" page
    And the user clicks on "echocardiogram test components" element on "Btris/Portal" page
    #Clcik on the select report button.
    And the user clicks on "btris: bottom select button" element on "Btris/Portal" page
    ...
    Examples:
      | Subject       | User_Name   | Filtered Subject | Subject 1     | Meds Order Start Date | Echocardiogram Tests                                                   | Echocardiogram Test Components      |
      | BTRIS-TEST-04 | btris_test2 | NIHCCTEST        | BTRIS-TEST-03 | 4/12/2006             | Modified Bruce Protocol Exercise Tolerance Echocardiography, Procedure | Amended On, Echocardiography Report |
