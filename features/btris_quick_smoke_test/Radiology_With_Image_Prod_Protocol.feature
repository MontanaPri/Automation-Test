...
Feature: BTRIS Radiology Test Report
  ...
  #Login to the system.
    ...
    And the user remembers the value of "radiology report page" field into "Results Preview - Page" on "Btris/Portal" page
    ...
    And the user clicks on "link to radiology image" element on "Btris/Portal" page
    And the switch to new tab page
    And the user must see "Date of Birth:" text in "radiology image" field on "Btris/Portal" page
    ...
    Examples:
      | Subject   | User_Name   |
      | 00-C-0069 | btris_test2 |


  @Radiology_Smoke_Test2

  Scenario Outline: 1.1 Radiology report including custom test list template.
  #Login to the system.
    Given the user navigates to / on browser
    And the user must see "Log in to BTRIS" text in "btris home: landing page" field on "Btris/Portal" page
    #Login to the BTRIS system.
    ...
    And the user clicks the "btris: select subject button" button on "Btris/Portal" page
    And the user enters "<Subject>" into "fine protocol and subject search box" on "Btris/Portal" page
    And the user verify that searched "<Subject>" is available
    And the user remembers the value of "protocol verify count" field into "Number OF Subjects" on "Btris/Portal" page
    And the user remembers the value of "selected protocol" field into "Protocol Number" on "Btris/Portal" page
