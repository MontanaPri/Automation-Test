...
@Laboratory_SC_Test_Smoke_Test
Feature: BTRIS Laboratory Shopping Cart Test Report


  @Laboratory_Smoke_TestSC1

  Scenario Outline: 1.1 Laboratory Shopping Cart Test Reports.
  #Login to the system.
    ...
    And the user remembers the value of "btris: laboratory filter report page" field into "Criteria Page" on "Btris/Portal" page
    And the user must see "Laboratory Criteria" text in "btris: laboratory filter report page" field on "Btris/Portal" page
    And the user enters "<Lab Collection Start Date>" into "lab collection start date" on "Btris/Portal" page
    And the user enters "Todays Date" into "lab collection end date" on "Btris/Portal" page
    #Clcik on the select report button.
    And the user clicks on "btris: top select subject button" element on "Btris/Portal" page
    #Enter and filter subject.
    And the user enters "<Subject>" into "fine protocol and subject search box" on "Btris/Portal" page
    ...
    Examples:
      | Subject       | User_Name   | Subject 1     | Lab Collection Start Date |
      | BTRIS-TEST-03 | btris_test2 | BTRIS-TEST-03 | 5/15/2006                 |
